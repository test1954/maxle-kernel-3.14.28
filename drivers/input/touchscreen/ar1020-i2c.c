	...
	*/ 
	if (probeForIRQ)
	{
		...
		for (i=probeMin;i<probeMax;i++)
		{
			...
			/* set_irq_type(i, IRQ_TYPE_EDGE_RISING);  maxle: replaced by... */
	...
	//set_irq_type(priv->irq, IRQ_TYPE_EDGE_RISING);  maxle replaced by
...
{
	int retval;
	int mret; /* maxle added */
        printk("AR1020 I2C: ar1020_i2c_init - entry\n");
	strcpy(receiveBuffer,"");
	strcpy(sendBuffer,"");
	...
	mret = i2c_add_driver(&ar1020_i2c_driver);
        printk("AR1020 I2C: ar1020_i2c_init - exit (add_driver() ret=%d\n", mret);
	return mret;
}
