		    ...
		    struct mxc_dispdrv_setting *setting)
{
	...
	dev_dbg(dev, "::::::maxle: ldb_init()  chno=%d\n", chno);
	chan = &ldb->chan[chno];
		     ...
		     struct fb_info *fbi)
{
	...
	for (i = 0; i < ARRAY_SIZE(ldb->clk_ldb_di_choices); i++) {
		...
		if (i <= 1) {
			...
			pr_debug("%s: i=%d round rate=%ld  serial_clk=%ld\n", __func__, i, rate, serial_clk);
		} else {
			rate = clk_get_rate(parent);
			pr_debug("%s: i=%d get rate=%ld  serial_clk=%ld\n", __func__, i, rate, serial_clk);
		}
		if (rate > serial_clk)
...
/*  
 * select display timing from command line.  
 *  
 * use params:  
 *  lvds0_timing=index   or   lvds0_timing=index,data-width
 *  lvds1_timing=index   or   lvds1_timing=index,data-width
 *  
 * where index 0 is the first timing entry, data-width=18 or 24  
 *  
 */  
...
static int get_ldb_timing(int ch, int *datawidth)  
{  
    char *x, *s = saved_command_line;  
    static char buff[16];  
    int timing = ERR_NO_LVDS_TIMING, dw = 0;
    char *p;

    sprintf(buff, "lvds%d_timing=", ch);  
  
    if ((x = strstr(s, buff)) != NULL) {  
        timing = simple_strtoul (&x[13], &p, 10);
	if (*p == ',') {
           p++;
           dw = simple_strtoul (p, NULL, 10);
           if (dw != 18 && dw != 24) {
                dw = -1;
           }
        }
...
static int ldb_probe(struct platform_device *pdev)
{
	...
	int dw_cmdline;  /* data-width as set from cmdline */	
	char clkname[16];

	dev_dbg(dev, "::::::maxle: ldb_probe()\n");
	...
	for_each_child_of_node(np, child) {
		...
		/* maxle: lvds timing may be overridden via cmdline... */                
		timing_idx = get_ldb_timing(chan->chno, &dw_cmdline);  
		if (timing_idx != ERR_NO_LVDS_TIMING) {		
			if (dw_cmdline > 0)
				data_width = dw_cmdline;  /* override data_width */
			else if (dw_cmdline == -1)
        	      		dev_err(dev, "lvds%d: illegal data width %d, ignoring...\n", chan->chno, dw_cmdline);
			dev_info(dev, "lvds%d: timing index=%d, data_width=%d\n", chan->chno, timing_idx, data_width );
		}
	...
	dev_dbg(dev, "::::::maxle: ldb_probe() - exit\n");
	return 0;
}
